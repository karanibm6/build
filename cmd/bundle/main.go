...
func Do(ctx context.Context) error {
	...
	if flagValues.prune {
		...
		if err := Prune(ctx, flagValues.image, auth); err != nil {
			return err
		}
	}
...
//
func Prune(ctx context.Context, image string, auth authn.Authenticator) error {
	...
	case "index.docker.io":
		...
		default:
			log.Printf("Removing a specific image tag is not supported on %s, the respective image tag will be overwritten with an empty image.\n", ref.Context().RegistryStr())
